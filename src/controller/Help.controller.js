...
export const GetHelp = AsyncHandler(async (req, res) => {
    ...
    const data = await HelpAndQuiteModal.find({ type: "Help" }).sort({ _id: -1 }).skip(skip).limit(limits);
...
export const GetQuites = AsyncHandler(async (req, res) => {
    ...
    const data = await HelpAndQuiteModal.find({ type: "Quites" }).sort({ _id: -1 }).skip(skip).limit(limits);
    return res.status(200).json({
        message: "helps",
        data
    })
